...
test_that('krige_rk execution', {
  ...
  preds_multi_numeric <- multi_predict(fitted, new_data = meuse.sf, neighbors = 4)
  ...
  preds_multi_conf_int <- multi_predict(fitted, new_data = meuse.sf, type = "conf_int", neighbors = 4)
